...
ULONG STDMETHODCALLTYPE HookedRelease(IUnknown* pUnk) {
    ...
    if (refCount == 0) {
        ...
        if (g_HookedVTables.count(pUnk)) {
            ...
            Logger::info("DX Hooks D3D10: Object " + std::to_string((uintptr_t)pUnk) + " destroyed. Removed from tracking.");
        }
    }
    return refCount;
}
...
HRESULT STDMETHODCALLTYPE HookedQueryInterface(IUnknown* pUnk, REFIID riid, void** ppvObject) {
    ...
    if (SUCCEEDED(hr) && ppvObject && *ppvObject) {
        ...
        if (isSwapChain) {
            Logger::info("DX Hooks D3D10: QueryInterface returned " + iidName + " interface at " + std::to_string((uintptr_t)*ppvObject));
...
HRESULT STDMETHODCALLTYPE HookedPresent1(
    ...
    if (s_present1Calls++ % 60 == 0) {
        Logger::info("HookedPresent1 D3D10 Entry [" + std::to_string(s_present1Calls) + "] SC=" + std::to_string((uintptr_t)pSwapChain));
    }
    g_frameCount++;
    OnDXPresent(pSwapChain);
    return g_OriginalPresent1(pSwapChain, SyncInterval, PresentFlags, pPresentParameters);
...
HRESULT STDMETHODCALLTYPE HookedResizeBuffers(
    ...
    Logger::info("HookedResizeBuffers D3D10 Entry [SC=" + std::to_string((uintptr_t)pSwapChain) + "]");
    g_IsResizing = true;
    ...
    if (FAILED(hr)) {
        ...
        Logger::error("HookedResizeBuffers D3D10: FAILED with HR=" + std::string(hex));
    } else {
        Logger::info("HookedResizeBuffers D3D10: Success");
    }
    return hr;
}
...
HRESULT STDMETHODCALLTYPE HookedResizeBuffers1(IDXGISwapChain3* pSwapChain, UINT BufferCount, UINT Width, UINT Height,
    ...
    Logger::info("HookedResizeBuffers1 D3D10 Entry [SC=" + std::to_string((uintptr_t)pSwapChain) + "]");
    g_IsResizing = true;
    OnDXResize(pSwapChain);
    ...
    if (FAILED(hr)) {
        char hex[16];
        sprintf(hex, "0x%08X", (unsigned int)hr);
        Logger::error("HookedResizeBuffers1 D3D10: FAILED with HR=" + std::string(hex));
    } else {
        Logger::info("HookedResizeBuffers1 D3D10: Success");
    }
    return hr;
}
...
void ApplySwapChainHooks(void* pSwapChain) {
    ...
    Logger::info("DX Hooks D3D10: Patching Instance VTable for SC=" + std::to_string((uintptr_t)pSwapChain) +
    ...
    if (VirtualProtect(pVTable, 128 * sizeof(void*), PAGE_READWRITE, &old)) {
        ...
        Logger::info("DX Hooks D3D10: Global SwapChain VTable entries patched (STEALTH MODE)");
    } else {
        Logger::error("DX Hooks D3D10: FAILED to VirtualProtect VTable for stealth patching!");
    }
}
...
HRESULT STDMETHODCALLTYPE HookedCreateSwapChain(
    ...
    Logger::info("DX Factory D3D10: CreateSwapChain triggered");
    HRESULT hr = g_OriginalCreateSwapChain(pFactory, pDevice, pDesc, ppSwapChain);
    if (SUCCEEDED(hr) && ppSwapChain && *ppSwapChain) {
        Logger::info("DX Factory D3D10: New SwapChain created at " + std::to_string((uintptr_t)*ppSwapChain));
...
HRESULT STDMETHODCALLTYPE HookedCreateSwapChainForHwnd(IDXGIFactory2* pFactory, IUnknown* pDevice, HWND hWnd,
    ...
    Logger::info("DX Factory D3D10: CreateSwapChainForHwnd triggered (HWND=" + std::to_string((uintptr_t)hWnd) + ")");
    HRESULT hr = g_OriginalCreateSwapChainForHwnd(pFactory, pDevice, hWnd, pDesc, pFullscreenDesc, pRestrictToOutput, ppSwapChain);
    if (SUCCEEDED(hr) && ppSwapChain && *ppSwapChain) {
        Logger::info("DX Factory D3D10: New SwapChain (Hwnd) created at " + std::to_string((uintptr_t)*ppSwapChain));
        ApplySwapChainHooks(*ppSwapChain);
    }
    return hr;
}
...
HRESULT STDMETHODCALLTYPE HookedCreateSwapChainForComposition(IDXGIFactory2* pFactory, IUnknown* pDevice,
    ...
    Logger::info("DX Factory D3D10: CreateSwapChainForComposition triggered");
    HRESULT hr = g_OriginalCreateSwapChainForComposition(pFactory, pDevice, pDesc, pRestrictToOutput, ppSwapChain);
    if (SUCCEEDED(hr) && ppSwapChain && *ppSwapChain) {
        Logger::info("DX Factory D3D10: New SwapChain (Composition) created at " + std::to_string((uintptr_t)*ppSwapChain));
        ApplySwapChainHooks(*ppSwapChain);
    }
    return hr;
}
...
HRESULT WINAPI HookedCreateDXGIFactory(REFIID riid, void** ppFactory) {
    ...
    ScopedRecursionGuard guard;
    Logger::info("DX Hooks D3D10: CreateDXGIFactory called");
...
HRESULT WINAPI HookedCreateDXGIFactory1(REFIID riid, void** ppFactory) {
    ...
    ScopedRecursionGuard guard;
    Logger::info("DX Hooks D3D10: CreateDXGIFactory1 called");
...
HRESULT WINAPI HookedCreateDXGIFactory2(UINT Flags, REFIID riid, void** ppFactory) {
    ...
    ScopedRecursionGuard guard;
    Logger::info("DX Hooks D3D10: CreateDXGIFactory2 called (Flags=" + std::to_string(Flags) + ")");
...
void HookDXGIFactories() {
    ...
    if (dxgi) {
        ...
        MH_EnableHook(MH_ALL_HOOKS);
        Logger::info("DX Hooks D3D10: Global DXGI Factory entry points hooked");
    }
}
