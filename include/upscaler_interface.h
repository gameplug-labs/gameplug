...
struct GamePlugUpscalerInterface {
    ...
    void (__cdecl *OnInit)(ImGuiContext* imguiContext, 
                          void (__cdecl *log)(LogLevel level, const char* message, void* context), 
                          void* logContext, 
                          uintptr_t instance, 
                          uintptr_t physicalDevice, 
                          uintptr_t device, 
                          uintptr_t queue, 
                          uintptr_t queueFamilyIndex, 
    ...
    void (__cdecl *OnRenderFrame)(uintptr_t commandBuffer, 
                                 uint64_t source, 
                                 uint64_t target,
...
namespace GamePlug {
    ...
    class Logger {
    public:
        static void info(const char* msg)  { if (get_log()) get_log()(GamePlugUpscalerInterface::LOG_INFO, msg, get_ctx()); }
        static void warn(const char* msg)  { if (get_log()) get_log()(GamePlugUpscalerInterface::LOG_WARN, msg, get_ctx()); }
        static void error(const char* msg) { if (get_log()) get_log()(GamePlugUpscalerInterface::LOG_ERROR, msg, get_ctx()); }
        static void debug(const char* msg) { if (get_log()) get_log()(GamePlugUpscalerInterface::LOG_DEBUG, msg, get_ctx()); }
