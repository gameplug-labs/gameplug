...
void ImageTracker::TrackImage(VkImage image, const VkImageCreateInfo* pCreateInfo) {
    if (!pCreateInfo || image == VK_NULL_HANDLE) return;
    ...
    if (trackCount++ < 500) {
        std::stringstream ss;
        ss << "ImageTracker: Tracked Image " << (void*)image 
           << " [" << pCreateInfo->extent.width << "x" << pCreateInfo->extent.height << "]"
           << " Format: " << pCreateInfo->format 
           << " Usage: 0x" << std::hex << pCreateInfo->usage;
        Logger::debug(ss.str());
    }
    ...
    if (pCreateInfo->usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        float score = 0.0f;
        if (m_screenWidth > 0 && pCreateInfo->extent.width == m_screenWidth && pCreateInfo->extent.height == m_screenHeight) score += 1000000.0f;
        if (pCreateInfo->extent.width != pCreateInfo->extent.height) score += 500000.0f;
        if (pCreateInfo->extent.width >= 640 && pCreateInfo->extent.height >= 360) score += 100000.0f;
        
        // Shadow map penalty
        if (pCreateInfo->extent.width == pCreateInfo->extent.height && m_screenWidth != m_screenHeight) score -= 800000.0f;
        // Minimum size filter
        if (pCreateInfo->extent.width < 320 || pCreateInfo->extent.height < 200) score = -100.0f;
        ...
        if (score >= m_bestDepthScorePerRes[resKey] && score > 0) {
            ...
            Logger::info("ImageTracker: Depth buffer candidate for " + std::to_string(pCreateInfo->extent.width) + "x" + std::to_string(pCreateInfo->extent.height) + " updated (Score=" + std::to_string(score) + ")");
        }
    }
}
...
void ImageTracker::UntrackImage(VkImage image) {
    ...
    for (auto it = m_views.begin(); it != m_views.end(); ) {
        if (it->second == image) it = m_views.erase(it);
        else ++it;
    }
}
...
void ImageTracker::TrackFramebuffer(VkFramebuffer fb, uint32_t count, const VkImageView* pViews) {
    if (fb == VK_NULL_HANDLE || !pViews || count == 0) return;
    ...
    for (VkImage img : resolvedImages) {
        ...
        if (info.usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
            float score = 0.0f;
            if (m_screenWidth > 0 && info.extent.width == m_screenWidth && info.extent.height == m_screenHeight) score += 1000000.0f;
            if (info.extent.width != info.extent.height) score += 500000.0f;
            if (info.extent.width >= 640 && info.extent.height >= 360) score += 100000.0f;

            // Shadow map penalty
            if (info.extent.width == info.extent.height && m_screenWidth != m_screenHeight) score -= 800000.0f;
            // Filter smaller buffers
            if (info.extent.width < 320 || info.extent.height < 200) score = -100.0f;
            ...
            if (score >= m_bestDepthScorePerRes[resKey] && score > 0) {
                ...
                depthCand = img;
                Logger::info("ImageTracker: Depth buffer best candidate for " + std::to_string(info.extent.width) + "x" + std::to_string(info.extent.height) + " updated (Score=" + std::to_string(score) + ")");
    ...
    if (depthCand != VK_NULL_HANDLE) {
        ...
        for (VkImage img : resolvedImages) {
            if (img == VK_NULL_HANDLE || img == depthCand) continue;
            if (m_images.find(img) == m_images.end()) continue;

            auto& info = m_images[img];
            float score = 0.0f;
            bool sizeMatch = (info.extent.width == dInfo.extent.width && info.extent.height == dInfo.extent.height);
            if (sizeMatch) score += 500000.0f;

            // Format scoring
            if (info.format == VK_FORMAT_R16G16_SFLOAT || info.format == VK_FORMAT_R16G16B16A16_SFLOAT) score += 200000.0f;
            else if (info.format == VK_FORMAT_R16G16_SNORM || info.format == VK_FORMAT_R16G16_UNORM) score += 150000.0f;
            else if (info.format == VK_FORMAT_R8G8_UNORM) score += 50000.0f;
            ...
            if (score >= m_bestMVScorePerRes[resKey] && score > 0) {
                ...
                mvCand = img;
                Logger::info("ImageTracker: Motion Vector best candidate for " + std::to_string(info.extent.width) + "x" + std::to_string(info.extent.height) + " updated");
...
bool ImageTracker::IsSceneFramebuffer(VkFramebuffer fb, uint32_t renderW, uint32_t renderH) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    if (m_framebuffers.find(fb) == m_framebuffers.end()) return false;
...
VkImage ImageTracker::GetSwapchainImageFromFramebuffer(VkFramebuffer fb) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    if (m_framebuffers.find(fb) == m_framebuffers.end()) return VK_NULL_HANDLE;
